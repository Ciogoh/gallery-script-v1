...
function setup() {
  ...
  // --- INIZIALIZZAZIONE: TESTO DI DEFAULT ---
...
function draw() {
  ...
  // 1. GESTIONE SFONDO E MODALITÀ COLORE
  if (bgImage) {
      // --- MODO IMMAGINE (Lente negativa) ---
      image(bgImage, 0, 0, width, height);
      blendMode(DIFFERENCE); 
      contentColor = color(255); // Bianco per l'effetto negativo
  } else {
      // --- MODO DEFAULT (Senza immagine) ---
      background('#FFFFFF');    
      blendMode(BLEND); 
...
class CustomShape {
  ...
  isHit(mx, my) {
    ...
    if (this.type === 'Text' && this.content.length > 0) {
        ...
        let singleLineWidth = (this.size * 0.5) * this.content.length;
        let maxBoxWidth = width - 40;
